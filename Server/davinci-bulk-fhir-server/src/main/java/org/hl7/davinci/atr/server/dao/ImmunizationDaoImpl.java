...
public class ImmunizationDaoImpl extends AbstractDao implements ImmunizationDao {
	...
	public DafImmunization getImmunizationById(int id) {
		List<DafImmunization> list = getSession().createNativeQuery(
				"select * from immunization where data->>'id' = '"+id+
				"' order by data->'meta'->>'versionId' desc", DafImmunization.class)
					.getResultList();
		return list.get(0);
	}
	...
	public DafImmunization getImmunizationByVersionId(int theId, String versionId) {
		return getSession().createNativeQuery(
			"select * from immunization where data->>'id' = '"+theId+
			"' and data->'meta'->>'versionId' = '"+versionId+"'", DafImmunization.class)
				.getSingleResult();
	}
	...
	public DafImmunization createImmunization(Immunization theImmunization) {
		DafImmunization dafImmunization = new DafImmunization();
		IParser jsonParser = fhirContext.newJsonParser();
		jsonParser.encodeResourceToString(theImmunization);
		dafImmunization.setData(jsonParser.encodeResourceToString(theImmunization));
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.save(dafImmunization);
	...
	public DafImmunization updateImmunizationById(int theId, Immunization theImmunization) {
		DafImmunization dafImmunization = new DafImmunization();
		IParser jsonParser = fhirContext.newJsonParser();
		dafImmunization.setId(theId);
		dafImmunization.setData(jsonParser.encodeResourceToString(theImmunization));
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.update(dafImmunization);
		session.getTransaction().commit();
		session.close();
		return dafImmunization;
	}
	...
	@Override
	public List<DafImmunization> getImmunizationForPatientsBulkData(String patientId, Date start, Date end){
	...
	@SuppressWarnings({"unchecked", "deprecation"})
	@Override
	public List<DafImmunization> getImmunizationForBulkData(Date start, Date end){
