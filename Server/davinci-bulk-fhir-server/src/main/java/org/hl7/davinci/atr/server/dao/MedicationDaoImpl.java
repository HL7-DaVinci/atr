...
public class MedicationDaoImpl extends AbstractDao implements MedicationDao {
	...
	public DafMedication getMedicationById(int id) {
		List<DafMedication> list = getSession().createNativeQuery(
				"select * from medication where data->>'id' = '"+id+
				"' order by data->'meta'->>'versionId' desc", DafMedication.class)
				.getResultList();
				return list.get(0);
	}
	...
	public DafMedication getMedicationByVersionId(int theId, String versionId) {
		return getSession().createNativeQuery(
				"select * from medication where id = '"+theId+
				"' and data->'meta'->>'versionId' = '"+versionId+"'", 
				DafMedication.class).getSingleResult();
	}
	...
	public DafMedication createMedication(Medication theMedication) {
		...
		DafMedication dafMedication = new DafMedication();
		IParser jsonParser = fhirContext.newJsonParser();;
		dafMedication.setData(jsonParser.encodeResourceToString(theMedication));
		session.beginTransaction();
		session.save(dafMedication);
	...
	public DafMedication updateMedicationById(int theId, Medication theMedication) {
		DafMedication dafMedication = new DafMedication();
		IParser jsonParser = fhirContext.newJsonParser();
		dafMedication.setId(theId);
		dafMedication.setData(jsonParser.encodeResourceToString(theMedication));
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.update(dafMedication);
		session.getTransaction().commit();
		session.close();
		return dafMedication;
	}
	...
	@Override
	public List<DafMedication> getMedicationForBulkData(List<String> patients, Date start, Date end){
