...
public class MedicationRequestDaoImpl extends AbstractDao implements MedicationRequestDao {
	...
	public DafMedicationRequest getMedicationRequestById(int id) {
		List<DafMedicationRequest> list = getSession().createNativeQuery(
			"select * from medicationrequest where data->>'id' = '"+id+
				"' order by data->'meta'->>'versionId' desc", DafMedicationRequest.class)
						.getResultList();
		return list.get(0);
    }
	...
	public DafMedicationRequest getMedicationRequestByVersionId(int theId, String versionId) {
		return getSession().createNativeQuery(
			"select * from medicationrequest where data->>'id' = '"+theId+
				"' and data->'meta'->>'versionId' = '"+versionId+"'", DafMedicationRequest.class)
					.getSingleResult();
	}
	...
	public DafMedicationRequest createMedicationRequest(MedicationRequest theMedicationRequest) {
		...
		DafMedicationRequest dafMedicationRequest = new DafMedicationRequest();
		IParser jsonParser = fhirContext.newJsonParser();;
		dafMedicationRequest.setData(jsonParser.encodeResourceToString(theMedicationRequest));
		session.beginTransaction();
		session.save(dafMedicationRequest);
	...
	public DafMedicationRequest updateMedicationRequestById(int theId, MedicationRequest theMedicationRequest) {
		DafMedicationRequest dafMedicationRequest = new DafMedicationRequest();
		IParser jsonParser = fhirContext.newJsonParser();
		dafMedicationRequest.setId(theId);
		dafMedicationRequest.setData(jsonParser.encodeResourceToString(theMedicationRequest));
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.update(dafMedicationRequest);
		session.getTransaction().commit();
		session.close();
		return dafMedicationRequest;
	}
	...
	@Override
	public List<DafMedicationRequest> getMedicationRequestForBulkData(Date start, Date end){
	...
	@SuppressWarnings({"unchecked", "deprecation"})
	@Override
	public List<DafMedicationRequest> getMedicationRequestForPatientsBulkData(String id, Date start, Date end){
