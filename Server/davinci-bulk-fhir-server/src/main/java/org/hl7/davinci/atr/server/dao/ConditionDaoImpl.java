...
public class ConditionDaoImpl extends AbstractDao implements ConditionDao {
	...
	@Override
	public DafCondition creatCondition(Condition theCondition) {
	...
	public DafCondition updateConditionById(int theId, Condition theCondition) {
		DafCondition dafCondition = new DafCondition();
		IParser jsonParser = fhirContext.newJsonParser();
		dafCondition.setId(theId);
		dafCondition.setData(jsonParser.encodeResourceToString(theCondition));
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.update(dafCondition);
		session.getTransaction().commit();
		session.close();
		return dafCondition;
	}
	...
	public List<DafCondition> getConditionForPatientsBulkData(String patientId, Date start, Date end){
	...
	@SuppressWarnings({"unchecked", "deprecation"})
	public List<DafCondition> getConditionForBulkData(Date start, Date end){
