...
public class CarePlanDaoImpl extends AbstractDao implements CarePlanDao {
	...
	public DafCarePlan getCarePlanById(int id) {
		List<DafCarePlan> list = getSession().createNativeQuery("select * from careplan where data->>'id' = '"+id+
				"' order by data->'meta'->>'versionId' desc", DafCarePlan.class)
				.getResultList();
		return list.get(0);
	}
	...
	/**
	 * This method builds criteria for fetching particular version of 
	 * the careplan record by id.
	 * 
	...
	public DafCarePlan getCarePlanByVersionId(int theId, String versionId) {
		return getSession().createNativeQuery(
			"select * from careplan where data->>'id' = '"+theId+
			"' and data->'meta'->>'versionId' = '"+versionId+"'", DafCarePlan.class)
				.getSingleResult();
	}
	...
	/**
     * This method builds criteria for creating the careplan
     * 
     * @param theCarePlan
     * @return : CarePlan record
     */
    @Override
	@Transactional
	public DafCarePlan createCarePlan(CarePlan theCarePlan) {
		Session session = sessionFactory.openSession();
		DafCarePlan dafCarePlan = new DafCarePlan();
		IParser jsonParser = fhirContext.newJsonParser();;
		dafCarePlan.setData(jsonParser.encodeResourceToString(theCarePlan));
		session.beginTransaction();
		session.save(dafCarePlan);
	...
	public DafCarePlan updateCarePlanById(int theId, CarePlan theCarePlan) {
		DafCarePlan dafCarePlan = new DafCarePlan();
		IParser jsonParser = fhirContext.newJsonParser();
		dafCarePlan.setId(theId);
		dafCarePlan.setData(jsonParser.encodeResourceToString(theCarePlan));
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.update(dafCarePlan);
		session.getTransaction().commit();
		session.close();
		return dafCarePlan;
	}
    ...
    @Override
    public List<DafCarePlan> getCarePlanForPatientsBulkData(String patientId, Date start, Date end){
	...
	@SuppressWarnings({"unchecked", "deprecation"})
    @Override
    public List<DafCarePlan> getCarePlanForBulkData(Date start, Date end){
