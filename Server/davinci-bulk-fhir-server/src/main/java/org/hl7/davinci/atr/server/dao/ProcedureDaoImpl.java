...
public class ProcedureDaoImpl extends AbstractDao implements ProcedureDao {
	...
	public DafProcedure getProcedureById(int id) {
		List<DafProcedure> list = getSession().createNativeQuery(
				"select * from procedure where data->>'id' = '"+id+
				"' order by data->'meta'->>'versionId' desc", DafProcedure.class)
					.getResultList();
			return list.get(0);	}
	...
	public DafProcedure getProcedureByVersionId(int theId, String versionId) {
		return getSession().createNativeQuery(
				"select * from procedure where data->>'id' = '"+theId+
				"' and data->'meta'->>'versionId' = '"+versionId+"'", DafProcedure.class)
					.getSingleResult();
	}
	...
	/**
     * This method builds criteria for creating the Procedure
	...
	public DafProcedure updateProcedureById(int theId, Procedure theProcedure) {
		DafProcedure dafProcedure = new DafProcedure();
		IParser jsonParser = fhirContext.newJsonParser();
		dafProcedure.setId(theId);
		dafProcedure.setData(jsonParser.encodeResourceToString(theProcedure));
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.update(dafProcedure);
		session.getTransaction().commit();
		session.close();
		return dafProcedure;
	}
	...
	public List<DafProcedure> getProcedureForBulkData(Date start, Date end){
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<DafProcedure> getProcedureForPatientsBulkData(String id, Date start, Date end) {
		@SuppressWarnings("deprecation")
		Criteria criteria = getSession().createCriteria(DafProcedure.class);
		if(id != null) {
