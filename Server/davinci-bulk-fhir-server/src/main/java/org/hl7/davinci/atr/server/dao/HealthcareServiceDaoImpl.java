...
public class HealthcareServiceDaoImpl extends AbstractDao implements HealthcareServiceDao{
	 ...
	 * @return : DAF object of the HealthcareService
	 */
	public DafHealthcareService getHealthcareServiceById(int id) {
		List<DafHealthcareService> list = getSession().createNativeQuery(
				"select * from HealthcareService where data->>'id' = '" + id +
				"' order by data->'meta'->>'versionId' desc",
				DafHealthcareService.class).getResultList();
		return list.get(0);
	}

	/**
	 * This method builds criteria for fetching particular version of the HealthcareService
	 * record by id.
	 * 
	...
	public DafHealthcareService getHealthcareServiceByVersionId(int theId, String versionId) {
		return getSession()
				.createNativeQuery("select * from HealthcareService where data->>'id' = '" + theId
						+ "' and data->'meta'->>'versionId' = '" + versionId + "'", DafHealthcareService.class)
				.getSingleResult();
	}
	...
	public DafHealthcareService createHealthcareService(HealthcareService theHealthcareService) {
		DafHealthcareService dafHealthcareService = new DafHealthcareService();
		IParser jsonParser = fhirContext.newJsonParser();
		jsonParser.encodeResourceToString(theHealthcareService);
		dafHealthcareService.setData(jsonParser.encodeResourceToString(theHealthcareService));
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.save(dafHealthcareService);
		session.getTransaction().commit();
		session.close();
		return dafHealthcareService;
	}
	...
	public DafHealthcareService updateHealthcareServiceById(int theId, HealthcareService theHealthcareService) {
		DafHealthcareService dafHealthcareService = new DafHealthcareService();
		IParser jsonParser = fhirContext.newJsonParser();
		dafHealthcareService.setId(theId);
		dafHealthcareService.setData(jsonParser.encodeResourceToString(theHealthcareService));
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.update(dafHealthcareService);
		session.getTransaction().commit();
		session.close();
		return dafHealthcareService;
	}
