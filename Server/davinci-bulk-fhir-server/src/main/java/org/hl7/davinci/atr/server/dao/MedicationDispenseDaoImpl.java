...
public class MedicationDispenseDaoImpl extends AbstractDao implements MedicationDispenseDao {
	...
	public DafMedicationDispense getMedicationDispenseById(int id) {
		List<DafMedicationDispense> list = getSession().createNativeQuery(
				"select * from medicationdispense where data->>'id' = '"+id+
				"' order by data->'meta'->>'versionId' desc", DafMedicationDispense.class)
				.getResultList();
				return list.get(0);
	}
	...
	public DafMedicationDispense getMedicationDispenseByVersionId(int theId, String versionId) {
		return getSession().createNativeQuery(
				"select * from medicationdispense where id = '"+theId+
				"' and data->'meta'->>'versionId' = '"+versionId+"'", 
				DafMedicationDispense.class).getSingleResult();
	}
	...
	public DafMedicationDispense createMedicationDispense(MedicationDispense theMedicationDispense) {
		...
		DafMedicationDispense dafMedicationDispense = new DafMedicationDispense();
		IParser jsonParser = fhirContext.newJsonParser();;
		dafMedicationDispense.setData(jsonParser.encodeResourceToString(theMedicationDispense));
		session.beginTransaction();
		session.save(dafMedicationDispense);
	...
	public DafMedicationDispense updateMedicationDispenseById(int theId, MedicationDispense theMedicationDispense) {
		DafMedicationDispense dafMedicationDispense = new DafMedicationDispense();
		IParser jsonParser = fhirContext.newJsonParser();
		dafMedicationDispense.setId(theId);
		dafMedicationDispense.setData(jsonParser.encodeResourceToString(theMedicationDispense));
		Session session = sessionFactory.openSession();
		session.beginTransaction();
		session.update(dafMedicationDispense);
		session.getTransaction().commit();
		session.close();
		return dafMedicationDispense;
	}
	...
	@Override
	public List<DafMedicationDispense> getMedicationDispenseForBulkData(Date start, Date end){
	...
	@SuppressWarnings({"unchecked", "deprecation"})
	@Override
	public List<DafMedicationDispense> getMedicationDispenseForPatientsBulkData(String id, Date start, Date end){
	    	
		Criteria criteria = getSession().createCriteria(DafMedicationDispense.class);
		if(id!=null) {
			criteria.add(Restrictions.sqlRestriction("{alias}.data->'subject'->>'reference' = 'Patient/"+id+"'"));
		}
		if(start != null) {
			criteria.add(Restrictions.ge("timestamp", start));
		}
