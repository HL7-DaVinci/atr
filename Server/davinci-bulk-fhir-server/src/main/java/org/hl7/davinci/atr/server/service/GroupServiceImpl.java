...
//	}
    ...
    public DafGroup processAddMemberToGroup(Parameters theParameters, String groupId) throws Exception {
        ...
        logger.info("Found Group Resource with row Id:::::{}",dafGroup.getId());
        if (dafGroup != null && StringUtils.isNotBlank(dafGroup.getData())) {
            ...
            if (group != null && !group.isEmpty() && theParameters != null && !theParameters.isEmpty()) {
                logger.info("Group resource is not null");
                if (group.hasExtension()) {
                    logger.info("Group has extensions");
                    for (Extension extension : group.getExtension()) {
                        if(extension.getUrl().toString().equals("http://hl7.org/fhir/us/davinci-atr/StructureDefinition/ext-attributionListStatus") && extension.getValue().toString().equals("final")){
                            throw new UnprocessableEntityException("The Attribution list status is 'final'. Modifications to the list is not allowed!");
                        }
                    }
    ...
    private Map<String, String> findProviderIdByIdentifier(Parameters theParameters) throws Exception {
        logger.info("Finding Provider by Identifier");
        ...
        logger.info("Found Provider by Identifier:::::{}",providerMap.size());
        return providerMap;
    }

    private String findPatientIdByIdentifier(Parameters theParameters) throws Exception {
        String patientId = null;
        logger.info("Finding Patient by Member Id");
        Identifier memberId = (Identifier) theParameters.getParameter(TextConstants.MEMBER_ID);
        ...
        logger.info("Found Patient by Member Id:::::{}",patientId);
        return patientId;
    }
    ...
    public DafGroup processRemoveMemberToGroup(Parameters theParameters, String groupId) throws Exception {
        ...
        if (dafGroup != null && StringUtils.isNotBlank(dafGroup.getData())) {
            ...
            if(group.hasExtension()){
                for(Extension extension: group.getExtension()){
                    if(extension.getUrl().toString().equals("http://hl7.org/fhir/us/davinci-atr/StructureDefinition/ext-attributionListStatus") && extension.getValue().toString().equals("finel")){
                        throw new UnprocessableEntityException("The Attribution list status is 'final'. Modifications to the list is not allowed!");
                    }
                }
                if (group != null && !group.isEmpty() && theParameters != null && !theParameters.isEmpty()) {
    ...
    @Override
    public Bundle processAttrStatus(Parameters theParameters, String groupId) throws Exception {
        Bundle bundle = new Bundle();
        DafGroup dafGroup = getGroupById(groupId);
        if (dafGroup != null) {
            Group group = parseGroup(dafGroup.getData());
            if (group.hasMember()) {
                List<GroupMemberComponent> membersList = group.getMember();
                GroupMemberComponent memberComponent = new GroupMemberComponent();
                for (GroupMemberComponent member : membersList) {
                    if (member.hasEntity()) {
                        Reference patientReference = member.getEntity();
                        String groupPatientId = patientReference.getReference().split("/")[1];
                        String parameterPatientId = findPatientIdByReference(theParameters);
                        if (groupPatientId.equals(parameterPatientId)) {
                            memberComponent = member;
                        }
                    }
                }

                if (memberComponent.hasEntity()) {
                    Reference patientReference = memberComponent.getEntity();
                    Patient patient = patientService.getPatientById(patientReference.getReference().split("/")[1]);
                    if (patient == null) {
                        throw new ResourceNotFoundException("Patient resource not found with Id::::::" + patientReference.getReference().split("/")[1]);
                    } else {
                        logger.info("Adding patient to Bundle");
                        bundle.getEntry().add(new Bundle.BundleEntryComponent().setResource(patient));
                        if (memberComponent.hasExtension()) {
                            List<Extension> memberExtensions = memberComponent.getExtension();
                            for (Extension extension : memberExtensions) {
                                if (extension.getUrl().equals("http://hl7.org/fhir/us/davinci-atr/StructureDefinition/ext-attributedProvider")) {
                                    Reference providerRef = (Reference) extension.getValue();
                                    String reference = providerRef.getReference();
                                    if (reference.split("/")[0].equals("Practitioner")) {
                                        ...
                                        if (practitioner == null) {
                                            ...
                                            logger.info("Adding practitioner to Bundle");
                                            bundle.getEntry().add(new Bundle.BundleEntryComponent().setResource(practitioner));
                                        }
                                    }
                                    if (reference.split("/")[0].equals("Organization")) {
                                        Organization organization = organizationService.getOrganizationById(reference.split("/")[1]);
                                        if (organization == null) {
                                            ...
                                            logger.info("Adding organization to Bundle");
                                            bundle.getEntry().add(new Bundle.BundleEntryComponent().setResource(organization));
                                        }
                                    }
                                    if (reference.split("/")[0].equals("PractitionerRole")) {
                                        PractitionerRole practitionerRole = practitionerRoleService.getPractitionerRoleById(reference.split("/")[1]);
                                        if (practitionerRole == null) {
                                            ...
                                            logger.info("Adding practitionerrole to Bundle");
                                            bundle.getEntry().add(new Bundle.BundleEntryComponent().setResource(practitionerRole));
                                        }
                                    }
                                }
                                if (extension.getUrl().equals("http://hl7.org/fhir/us/davinci-atr/StructureDefinition/ext-coverageReference")) {
                                    ...
                                    if (reference.split("/")[0].equals("Coverage")) {
                                        ...
                                        if (coverage == null) {
                                            ...
                                            logger.info("Adding coverage to Bundle");
                                            bundle.getEntry().add(new Bundle.BundleEntryComponent().setResource(coverage));
                                        }
                                    }
                                }
                            }
        ...
        logger.info("Bundle Resource::::{}", fhirContext.newJsonParser().encodeResourceToString(bundle));
        return bundle;
    }
}